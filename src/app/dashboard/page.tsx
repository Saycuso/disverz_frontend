...
export default function Dashboard() {
    ...
    const fetchAdminGuilds = async () => {
      try {
        ...
        if (res.ok) {
          ...
          console.warn("Silent Notice: Awaiting complete authentication session synchronization.");
        }
      } catch (error) {
        console.warn("Silent Notice: Network background refresh state cycle updated:", error);
      } finally {
  ...
  useEffect(() => {
    const currentAvailableIds = guilds
      .filter((guild) => !myServers.some((server) => server.discordId === guild.id))
      .map((g) => g.id);
    ...
    <div className="max-w-6xl mx-auto w-full">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8 border-b border-white/5 pb-6">
        ...
        <div className="mt-4 md:mt-0 bg-[#111] border border-white/5 px-4 py-2 rounded-lg text-xs text-gray-400 font-semibold flex items-center gap-2 select-none">
          ...
          Total Linked Guilds: <span className="text-white font-bold">{myServers.length}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-7">
          <div className="bg-[#111] border border-white/5 p-6 rounded-xl shadow-xl w-full">
            ...
            {isLoadingServers ? (
              ...
              <div className="border border-dashed border-white/10 p-10 rounded-xl text-center flex flex-col items-center justify-center h-60">
                ...
                <h3 className="text-gray-300 font-bold mb-1">No Servers Listed Yet</h3>
                <p className="text-sm text-gray-500 max-w-sm">
                  Select a server from the left panel and click {"List Server on Disverz"} to begin dominating the ranks.
                </p>
              </div>
            ) : (
                  ...
                  >
                    <div className="flex items-start gap-3 mb-4">
                      ...
                      <div className="-hidden">
                        <h3 className="font-bold text-sm leading-tight text-white group-hover:text-orange-500 transition-colors truncate">{server.name}</h3>
                        <p className="text-[11px] text-gray-500 mt-1 flex items-center gap-1">
                          <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                          {server.lastHumanMsgAt ? new Date(server.lastHumanMsgAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : "Active"}
                        </p>
                      </div>
                    </div>
